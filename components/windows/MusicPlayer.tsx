// AI GENERATED
import { RiSkipForwardFill, RiSkipBackFill, RiPlayFill, RiPauseFill } from "react-icons/ri";
import DragWindow from "../DragWindow";
import { useMusicContext } from "@/providers/musicProvider";
import { useEffect, useState, useRef } from "react";

export function MusicPlayerWindow() {
    const { currentTrack, isMusicMuted, toggleMusicMute, nextTrack, prevTrack, audioRef } = useMusicContext();
    ...
    const handleSeekCommit = () => {
        ...
        // 如果目前不是靜音（處於播放狀態），則在拖曳改變時間後強制重新觸發播放
        // 以避免某些瀏覽器在手動設定 currentTime 時自動切換為暫停狀態
        if (!isMusicMuted) {
            audioRef.current.play().catch(e => console.warn("Seek play prevented:", e));
        }
    };
        ...
        <DragWindow title="MusicPlayer." id="musicplayer">
            <div className="flex flex-col gap-5 w-64 select-none">
                ...
                <div className="flex flex-row items-center justify-center gap-6 mb-2">
                    ...
                    >
                        {isMusicMuted ? <RiPlayFill size={40} /> : <RiPauseFill size={40} />}
                    </button>
